class Hand
  ...
  def get_value
    # get rank of cards and put them in an array
    # card_ranks = []
    # dealt_cards.each { |card| card_ranks.push(card.rank) }
    # card_ranks = dealt_cards.map {|card| card.rank} map & collect are interchangable, they can loop and push here
    card_ranks = dealt_cards.collect { |card| card.rank }
    # value = 0
    # card_ranks.each do |rank|
    #   rank = rank.to_sym #convert string to symbol
    #   value += VALUES[rank] # we will have VALUES hash
    # end
    # reduce, acc - 0, next iterayion sets acc to result of first iteration and does the calc
